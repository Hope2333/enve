...
Before substantial work, read `.ai/README.md`, `docs/ai-relay.md`, and then the area-specific handoff file it points to. When using a supervisory/execution AI split, also follow `docs/ai-collaboration.md`. After any meaningful state change, update the relevant handoff docs in the same work session so the next AI can continue without reconstructing context. For the current build and CI modernization lane, keep `.ai/modernization/ai-handoff.md` and `.ai/modernization/current-status.md` current with the latest branch, commit, workflow run, blocker, verification result, and next action. AI working-state docs and helper tools now live under `.ai/` and are local-only; do not move them back under `docs/` unless the human explicitly asks. For the local wait tool `.ai/tools/watch-build-status.sh`, the parameter order is `REPO RUN_ID INTERVAL`; do not swap `REPO` and `RUN_ID`. Do not commit `.omx/` or `.ai/`.
