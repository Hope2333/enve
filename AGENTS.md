...
Before substantial work, read `docs/ai-relay.md` and then the area-specific handoff file it points to. When using a supervisory/execution AI split, also follow `docs/ai-collaboration.md`. After any meaningful state change, update the relevant handoff docs in the same work session so the next AI can continue without reconstructing context. For the current build and CI modernization lane, keep `docs/modernization/ai-handoff.md` and `docs/modernization/current-status.md` current with the latest branch, commit, workflow run, blocker, verification result, and next action. For `scripts/ci/watch-build-status.sh`, the parameter order is `REPO RUN_ID INTERVAL`; do not swap `REPO` and `RUN_ID`. Do not commit `.omx/`.
